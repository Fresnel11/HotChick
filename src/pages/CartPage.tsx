  ...
  if (items.length === 0) {
    return (
      <main className="min-h-[70vh] flex flex-col items-center justify-center bg-muted">
        ...
        <h1 className="font-display text-3xl mb-2 uppercase">{t("cart.emptyTitle")}</h1>
        <p className="text-muted-foreground mb-6">{t("cart.emptyText")}</p>
        <Link
          to="/menu"
  ...
  return (
    <main className="py-12 md:py-20 bg-muted min-h-screen">
      <div className="container max-w-3xl">
        <h1 className="font-display text-4xl mb-8 uppercase">{t("cart.title")}</h1>
        ...
        <div className="bg-card p-6 rounded-xl shadow-sm border border-border/50">
          ...
          <Link
            to="/checkout"
            className="block w-full text-center bg-primary text-primary-foreground font-display tracking-wider py-4 rounded-xl text-lg hover:bg-primary/90 active:scale-[0.98] transition-all uppercase"
          >
            {t("cart.checkout")}
          </Link>
        </div>
