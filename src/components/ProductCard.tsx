...
const ProductCard = ({ item }: { item: MenuItem }) => {
  ...
  return (
    <div className="group bg-card rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:shadow-primary/10 border border-border/50 transition-all duration-500 hover:-translate-y-2">
      <div className="relative overflow-hidden aspect-[4/3]">
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-foreground/0 group-hover:bg-foreground/20 transition-all duration-500">
          <motion.button
            onClick={handleAdd}
            whileTap={{ scale: 0.9 }}
            className="opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300 bg-primary text-primary-foreground font-display tracking-wider px-6 py-3 rounded-xl shadow-xl"
          >
            ADD TO CART
          </motion.button>
        </div>
      ...
      <div className="p-5">
        <h3 className="font-display text-lg tracking-wide mb-1">{item.name}</h3>
